  ...
  factory: (require) => {
    ...
    function insertStyles() {
      ...
      const css = `
        ...
        .dsh-pet-status { position: absolute; bottom: calc(100% + 6px); right: 0; pointer-events: none; display: flex; flex-direction: column; align-items: flex-end; gap: 4px; max-width: 300px; }
        ...
        .dsh-pet-menu { position: absolute; bottom: calc(100% + 8px); right: 0; z-index: 10001; pointer-events: auto; width: 180px; padding: 10px; border-radius: 12px; background: rgba(24, 24, 27, 0.94); color: #f4f4f5; box-shadow: 0 10px 32px rgba(0, 0, 0, 0.4); font: 12px/1.6 system-ui, -apple-system, sans-serif; backdrop-filter: blur(10px); }
        .dsh-pet-menu .title { margin: 0 0 8px; font-size: 11px; font-weight: 600; letter-spacing: 0.02em; color: rgba(244, 244, 245, 0.6); }
        .dsh-pet-menu .pets { display: flex; gap: 8px; }
        .dsh-pet-menu .pet-btn { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 8px 4px; border: 1px solid rgba(255, 255, 255, 0.12); border-radius: 10px; background: rgba(255, 255, 255, 0.05); color: inherit; font: inherit; cursor: pointer; transition: background 0.12s ease, border-color 0.12s ease; }
